...
import { Users, Reports, Rewards, Collectedfloods, Notifications, Transactions,WeatherTable, zones } from './schema';
...
export async function createReport(
  ...
  try {
    ...
    // Award 10 points for reporting flood
    const pointsEarned = 10;
    await updateRewardPoints(userId, pointsEarned);

    // Create a transaction for the earned points
    await createTransaction(userId, 'earned_report', pointsEarned, 'Points earned for reporting flood');

    // Create a notification for the user
    await createNotification(
      userId,
      `You've earned ${pointsEarned} points for reporting flood!`,
      'reward'
    );
...
export async function createCollectedflood(reportId: number, collectorId: number, notes?: string) {
  try {
    const [collectedflood] = await db
      .insert(Collectedfloods)
      .values({
        reportId,
        collectorId,
      ...
      .returning()
      .execute();
    return collectedflood;
  } catch (error) {
    console.error("Error creating collected flood:", error);
    return null;
  }
}

export async function getCollectedfloodsByCollector(collectorId: number) {
  try {
    return await db.select().from(Collectedfloods).where(eq(Collectedfloods.collectorId, collectorId)).execute();
  } catch (error) {
    console.error("Error fetching collected floods:", error);
    return [];
  }
}
...
export async function getfloodCollectionTasks(limit: number = 20) {
  try {
    const tasks = await db
      .select({
        id: Reports.id,
        location: Reports.location,
        floodType: Reports.floodType,
    ...
    console.error("Error fetching flood collection tasks:", error);
    return [];
  }
}
...
export async function saveReward(userId: number, amount: number) {
  try {
    const [reward] = await db
      ...
      .values({
        userId,
        name: 'flood Collection Reward',
        collectionInfo: 'Points earned from flood collection',
        points: amount,
        level: 1,
        isAvailable: true,
      })
    ...
    await createTransaction(userId, 'earned_collect', amount, 'Points earned for collecting flood');
...
export async function saveCollectedflood(reportId: number, collectorId: number, verificationResult: any) {
  try {
    const [collectedflood] = await db
      .insert(Collectedfloods)
      .values({
    ...
    console.error("Error saving collected flood:", error);
    throw error;
  }
}
    ...
    const allRewards = [
      {
        ...
        collectionInfo: "Points earned from reporting and collecting flood"
      },
      ...dbRewards
    ];
...
export async function saveWeatherData(weatherData: any) {
  try {
...
export async function saveZone (lat:number, lng:number,riskLevel:string){
  ...
  try{
    ...
    console.error("error saving zones data: ", error);
    throw error;
  }
};

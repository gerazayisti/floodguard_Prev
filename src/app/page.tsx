...
import { getRecentReports, getAllRewards, getfloodCollectionTasks } from '@/utils/db/actions'
  ...
  return (
    <div className="relative w-32 h-32 mx-auto mb-8">
      ...
      <Image
              ...
              width={256} 
              height={256}
              className="absolute inset-0 m-auto h-32 w-32 text-blue-600 animate-none rounded-full"
            />
    </div>
  )
}
  ...
  useEffect(() => {
    async function fetchImpactData() {
      try {
        ...
        const tasks = await getfloodCollectionTasks(100);  // Fetch last 100 tasks

        const floodCollected = tasks.reduce((total, task) => {
        ...
        const co2Offset = floodCollected * 0.5;  // Assuming 0.5 kg CO2 offset per kg of flood

        setImpactData({
          floodCollected: Math.round(floodCollected * 10) / 10, // Round to 1 decimal place
          reportsSubmitted,
          tokensEarned,
    ...
    <div className={`container mx-auto px-4 py-16 ${poppins.className}`}>
      <section className="text-center mb-20">
        ...
        <h1 className="text-6xl font-bold mb-6 text-gray-800 tracking-tight">
          FloodGuard<br/> <span className="text-blue-600">flood Management</span>
        </h1>
        <p className="text-xl text-gray-600 max-w-2xl mx-auto leading-relaxed mb-8">
          Join our community in making flood management more efficient and rewarding!
        </p>
        {!loggedIn ? (
      ...
      <section className="grid md:grid-cols-3 gap-10 mb-20">
        <FeatureCard
          icon={Leaf}
          title="Eco-Friendly"
          description="Contribute to a cleaner environment by reporting and collecting flood."
        />
        <FeatureCard
          icon={Coins}
          title="Earn Rewards"
          description="Get tokens for your contributions to flood management efforts."
        />
        <FeatureCard
          icon={Users}
      ...
      <section className="bg-white p-10 rounded-3xl shadow-lg mb-20">
        ...
        <div className="grid md:grid-cols-4 gap-6">
          <ImpactCard title="flood Collected" value={`${impactData.floodCollected} kg`} icon={Recycle} />

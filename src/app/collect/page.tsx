...
import { getfloodCollectionTasks, updateTaskStatus, saveReward, saveCollectedflood, getUserByEmail } from '@/utils/db/actions'
...
export default function CollectPage() {
  ...
  const [hoveredfloodType, setHoveredfloodType] = useState<string | null>(null)
  ...
  useEffect(() => {
    const fetchUserAndTasks = async () => {
      ...
      try {
        ...
        // Fetch tasks
        const fetchedTasks = await getfloodCollectionTasks()
  ...
  const handleStatusChange = async (taskId: number, newStatus: CollectionTask['status']) => {
    if (!user) {
      toast.error('Please log in to collect flood.')
      return
    }
  ...
  const handleVerify = async () => {
    ...
    try {
      ...
      const prompt = `You are an expert in flood management and recycling. Analyze this image and provide:
        1. Confirm if the flood type matches: ${selectedTask.floodType}
        2. Estimate if the quantity matches: ${selectedTask.amount}
        3. Your confidence level in this assessment (as a percentage)
        
        Respond in JSON format like this:
        {
          "floodTypeMatch": true/false,
      ...
      try {
        ...
        setVerificationResult({
          floodTypeMatch: parsedResult.floodTypeMatch,
          quantityMatch: parsedResult.quantityMatch,
          confidence: parsedResult.confidence
        })
        setVerificationStatus('success')
        
        if (parsedResult.floodTypeMatch && parsedResult.quantityMatch && parsedResult.confidence > 0.7) {
          ...
          // Save the collected flood
          await saveCollectedflood(selectedTask.id, user.id, parsedResult)
          ...
          toast.error('Verification failed. The collected flood does not match the reported flood.', {
            duration: 5000,
            position: 'top-center',
          })
        }
  ...
  return (
    <div className="p-4 sm:p-6 lg:p-8 max-w-4xl mx-auto">
      <h1 className="text-3xl font-semibold mb-6 text-gray-800">Flood Collection Tasks</h1>
              ...
              <div key={task.id} className="bg-white p-4 rounded-lg shadow-sm border border-gray-200">
                ...
                <div className="grid grid-cols-3 gap-2 text-sm text-gray-600 mb-3">
                  <div className="flex items-center relative">
                    ...
                    <span 
                      onMouseEnter={() => setHoveredfloodType(task.floodType)}
                      onMouseLeave={() => setHoveredfloodType(null)}
                      className="cursor-pointer"
                    >
                      {task.floodType.length > 8 ? `${task.floodType.slice(0, 8)}...` : task.floodType}
                    </span>
                    {hoveredfloodType === task.floodType && (
      ...
      {selectedTask && (
        <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center p-4 z-50">
          <div className="bg-white rounded-lg p-6 max-w-2xl w-full max-h-[90vh] overflow-y-auto">
            ...
            <p className="mb-4 text-sm text-gray-600">Upload a photo of the collected flood to verify and earn your reward.</p>
            <div className="mb-4">
            ...
            {verificationStatus === 'success' && verificationResult && (
              <div className="mt-4 p-4 bg-blue-50 border border-blue-200 rounded-md">
                <p>flood Type Match: {verificationResult.floodTypeMatch ? 'Yes' : 'No'}</p>
      ...
      {/* {user ? (
        ...
        <p className="text-sm text-red-600 mb-4">Please log in to collect flood and earn rewards.</p>
      )} */}
    </div>
  )
}
